...
export async function buildPdfDocumentForSite(roomsData = []) {
  ...
  // Čitanje korisničkog loga i profila
  ...
  const userLogoDataUrl = localStorage.getItem(logoKey) || null;
  ...
  function drawHeader() {
    if (userLogoDataUrl) {
      try {
        doc.addImage(userLogoDataUrl, "PNG", 10, 6, 18, 18);
      } catch (e) {
        // preskoči ako ne uspije
      }
    }
  ...
  if (profile.companyName) {
    doc.text(`Izvođač: ${profile.companyName}`, 12, y2); y2 += 5;
  }
