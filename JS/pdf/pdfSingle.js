...
// Čita korisnički logo iz localStorage
function getUserLogoDataUrl() {
  ...
  return localStorage.getItem(logoKey) || null;
}

// Čita korisnički profil iz localStorage
...
export async function buildPdfDocument(data = {}) {
  ...
  const logoDataUrl = getUserLogoDataUrl();
  ...
  const drawFrame = () => {
    ...
    doc.setFontSize(8);
    doc.text(profile.companyName || '', frameX + 4, frameY + 6);
    doc.text(profile.owner || '', frameX + 4, frameY + 10);
